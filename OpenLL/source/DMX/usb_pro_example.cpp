...
// Read a DMX packet
uint8_t FTDI_RxDMX(int label, unsigned char *data, uint32_t expected_length)
{
...
void ReceiveDMX(int PortLabel)
{
	...
	if (device_handle != NULL)
	{
		...
		_getch();
		printf("\nSetting the widget to send DMX only when signal change... ");
		...
		for (int i = 0; i < counter ; i++)
		{
			...
			res = FTDI_RxDMX(PortLabel, myDmxIn, 513);
			if (res < 0)
			{
				printf("DMX Receive FAILED ... exiting \n");
				FTDI_ClosePort();
				break;
			}
			printf("\nDMX Data from 0 to 10: ");
...
void ReceiveMIDI(int PortLabel)
{
	...
	if (device_handle != NULL)
	{
		...
		{
			memset(MyData,0,3);
			res = FTDI_RxDMX(PortLabel, MyData, 3);
			if (res < 0)
			{
